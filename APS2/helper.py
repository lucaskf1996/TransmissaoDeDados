    ...
    def assembleDataPackets(self, data, messageID):
        ...
        for i, e in enumerate(content):
            ...
            if e is bytes:
                if i >=2:
                    troll = i+1    
                    out[str(i)] = self.constructParcel(head = headerBase + troll.to_bytes(2,"big") + len(content[i]).to_bytes(1,"big") + bytes([0,0,0,0]),data = e)
                else:
                    out[str(i)] = self.constructParcel(head = headerBase + i.to_bytes(2,"big") + len(content[i]).to_bytes(1,"big") + bytes([0,0,0,0]),data = e)
            else:
                if i >=2:
                    troll = i+1    
                    out[str(i)] = self.constructParcel(head = headerBase + troll.to_bytes(2,"big") + len(content[i]).to_bytes(1,"big") + bytes([0,0,0,0]),data = bytes(e))
                else:
